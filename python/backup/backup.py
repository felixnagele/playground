...
def fetch_public_repos(username: str) -> List[str]:
    """Fetch all public repos for a GitHub user."""
    repos = []
    url = f"https://api.github.com/users/{username}/repos"

    while url:
        resp = requests.get(url, params={"per_page": 100, "type": "public"})
...
def backup_repos(work_dir: Path, username: Optional[str] = None) -> int:
    if work_dir is None:
        work_dir = Path.cwd()
    ...
    if private_file.exists():
        ...
        print(f"\n⚠ No {private_file} found, skipping private repos")
...
if __name__ == "__main__":
    ...
    work_path = Path(args.work_dir) if args.work_dir else Path.cwd()
