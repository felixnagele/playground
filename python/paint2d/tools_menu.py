...
class ToolsMenu:
    ...
    def __init__(self, window_width: int, window_height: int, main_surface: pygame.Surface, paint_surface: pygame.Surface):
        ...
        self.tool_item_active = dict.fromkeys(self.tool_item_list, False) #Convert list to dict with all values on False
        self.tool_item_selected = dict.fromkeys(self.tool_item_list, False) #Convert list to dict with all values on False
        self.tool_item_hover = dict.fromkeys(self.tool_item_list, False) #Convert list to dict wtih all values on False
    ...
    def calculate_tool_coordinates(self):
        for i in range(len(self.tool_item_list)):
            self.tool_item_x.insert(i, self.window_width - self.tool_item_width - self.tool_item_gap)
            self.tool_item_y.insert(i, self.window_height - self.tool_item_height - self.tool_item_gap * (i+1))
    ...
    def draw(self, pygame: pygame):
        for i in range(len(self.tool_item_list)):
            ...
            image = pygame.transform.scale(image, (self.tool_item_width, self.tool_item_height))
    ...
    def draw_selected_tool(self, pygame: pygame):
        for i in range(len(self.tool_item_list)):
            if self.tool_item_selected[self.tool_item_list[i]]:
                ...
                image = pygame.transform.scale(image, (self.tool_item_width, self.tool_item_height))
                self.main_surface.blit(image, (self.get_selected_tool_location()[0], self.get_selected_tool_location()[1]))
    ...
    def draw_size_color_fields(self, pygame: pygame):
        gap_text_y = 7
        gap_text_x = 5
        pygame.draw.rect(self.main_surface, (0, 0, 0), (self.get_size_field_location()[0] - gap_text_x, self.get_size_field_location()[1] - self.size_field_height//2 + gap_text_y, self.size_field_width, self.size_field_height), self.border)
        pygame.draw.rect(self.main_surface, (0, 0, 0), (self.get_color_field_location()[0] - gap_text_x, self.get_color_field_location()[1] - self.color_field_height//2 + gap_text_y, self.color_field_width, self.color_field_height), self.border)

    # Collision detection for the tool items
    def collision(self, mouse_x: int, mouse_y: int):
        for i in range(len(self.tool_item_list)):
            if mouse_x >= self.tool_item_x[i] and mouse_x <= self.tool_item_x[i] + self.tool_item_width and mouse_y >= self.tool_item_y[i] and mouse_y <= self.tool_item_y[i] + self.tool_item_height:
    ...
    def collision_size_field(self, mouse_x: int, mouse_y: int) -> bool:
        ...
        if mouse_x >= size_x and mouse_x <= size_x + self.size_field_width and mouse_y >= size_y and mouse_y <= size_y + self.size_field_height:
            return True
        return False
    ...
    def collision_color_field(self, mouse_x: int, mouse_y: int) -> bool:
        ...
        if mouse_x >= color_x and mouse_x <= color_x + self.color_field_width and mouse_y >= color_y and mouse_y <= color_y + self.color_field_height:
            return True
        return False
    ...
    def get_size_field_location(self):
        ...
        field_gap = 75
        self.size_field_location_list.insert(0, self.window_width - self.tool_item_width - self.tool_item_gap - field_gap)
        self.size_field_location_list.insert(1, self.window_height - self.tool_item_height - self.tool_item_gap * position)
    ...
    def get_color_field_location(self):
        position = len(self.tool_item_list)+1
        field_gap = 30
        self.color_field_location_list.insert(0, self.window_width - self.tool_item_width - self.tool_item_gap - field_gap)
        self.color_field_location_list.insert(1, self.window_height - self.tool_item_height - self.tool_item_gap * position)
        return self.color_field_location_list

    def get_selected_tool_location(self):
        position = len(self.tool_item_list)+2
        self.selected_tool_location_list.insert(0, self.window_width - self.tool_item_width - self.tool_item_gap)
        self.selected_tool_location_list.insert(1, self.window_height - self.tool_item_height - self.tool_item_gap * position)
