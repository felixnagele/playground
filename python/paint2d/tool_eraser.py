...
class Eraser:
    ...
    # Append eraser data
    def append(self, mouse_x: int, mouse_y: int, size, drawn_history: dict, current_layer: int):
        if self.state:
            applied_size = 1
            ...
            #print(self.data)
            drawn_history.update({current_layer: ([(mouse_x, mouse_y), applied_size],self.name)})
